...
import { TimeDimensionExtraFields, TimeDimensionUpdater } from '@cubejs-client/react';
...
type TimeGroupProps = {
    members:  (Omit<TimeDimension, "dimension"> & TimeDimensionExtraFields & {
        index: number;
    })[] ,
    availableMembers: TCubeDimension[],
    addMemberName: string,
    title: string,
    updateMethods : TimeDimensionUpdater
  }
...
const TimeGroup : React.FC<TimeGroupProps>= ({
  ...
  const dateRangeMenu = (onClick: { (dateRange: any): void; (arg0: { title: string; value: undefined; } | { value: string; title?: undefined; }): void; }) => (
    <Menu>
      {DateRanges.map(m => (
